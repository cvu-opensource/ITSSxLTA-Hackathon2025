...
def generate_multiDAG(N=1000, P=10, K=2, s=20, L=5, T=100, P_task=None, P_id=None, N_task=None):
    ...
    g = {}
    h = {}
    fourier_basis = FourierBasis((0, 1), n_basis=K, period=1)
...
def net_fpca(h, K):
    ...
    for l in range(L):
        [N, P, T] = np.shape(h[l])
        a[l] = np.zeros((N, P, K))  # functional across 
        v[l] = np.zeros((P, K, T))  # 
        for j in range(P):
            # functional grid per column
            fdata_ij = FDataGrid(h[l][:, j, :], np.arange(0, 1, 1 / T))  #  (N, T, 1)
            fpca_grid = FPCA(K)  # 2 PCA squash
            if np.isnan(h[l][:, j, :]).any():
                continue
...
def multiDAG_functional(X, lambda1, rho, P_id, P_all, max_iter=200, alpha_max=1e+16, h_tol=1e-6, G_true=None):
    ...
    def _loss(G):
        G_Gloss = {}
        loss = 0
        # print('range(L)', range(L))
        for l in range(L):
            G_Gloss[l] = np.zeros((P[l] * K, P[l] * K))
            # print('shape in G_Gloss', [value.shape for thing, value in  G_Gloss.items()])
            M = X[l] @ G[l]
    ...
    def _h(G):
        W, G_W = _f(G)  # SOMETHING SOMETHING WEIGHTS
        h = 0
        p = 0
        E = {}
        G_hG = {}
    ...
    def _grad(g):
        ...
        # print('h', h)
        # print('p', p)
        # print('G_hG shape', [value.shape for thing, value in G_hG.items()])
        # print('G_pG shape', [value.shape for thing, value in G_pG.items()])
    ...
    g_est = _vec(Empty)  # g_est are our model parameters.
    ...
    for l in range(L):
        ...
        for i in range(P[l] * K):
            for j in range(P[l] * K):
                E_est[l][int(i / K), int(j / K)] += G_est[l][i, j] ** 2  # a^2 + b^2 + c^2 + ... 
        for i in range(P[l]):
            for j in range(P[l]):
                E_est[l][i, j] = np.sqrt(E_est[l][i, j])  # sqrt(a^2 + b^2 + c^2 + ...)
    return E_est, G_est
...
def single_test(N=None, K=None, P=None, n_task=None, rep=None):
    ...
    for rep_id in range(rep):
        ...
        for l in range(n_task):
            np.savetxt('./test_graphs/E_est_multi0_%i,task_%i.csv' % (n_task, l + n_task * rep_id), E_est[l])
            np.savetxt('./test_graphs/W_true_multi0_%i,task_%i.csv' % (n_task, l + n_task * rep_id), W_true[l + rep_id * n_task])
            ...
            niter += 1
    result_pd.to_csv('./test_graphs/N=%i,K=%i,P=%i,L=%i.csv' % (N, K, P, n_task))
