...
fp = '/mnt/d/ben/ITSSxLTA-Hackathon2025/district_DAG_lambda_0.001_rho_1_K_2.pkl'
...
class OurNetwork():
    ...
    def vis_digraph(self, network: np.ndarray, save_path: str, weight_thresh=0.1, dash_thresh=0.5):
        ...
        assert len(network.shape) == 2, f'Assertion failed, network recieved is not a 2d array.'
    ...
    def query_for_context(self, queries, weight_threshs) -> list[]

    def query_graph(self, query_nodes: list, weight_threshs: list | None = None) -> list[nx.Graph]:
        subgraphs = []
        if weight_threshs is None:
            weight_threshs = [0.1] * len(query_nodes)

        for w_t, q in zip(query_nodes, weight_threshs):
            ...
            subg.add_node(q)
            neighbors = self.graph.neighbors(q) #Find all neighbors of node n

            for neighs in neighbors:
                weight_in_graph = self.graph[n][neighs]['weight']
                if weight_in_graph > w_t:
                    subg.add_edge(n, neighs, weight=weight_in_graph)
...
network = OurNetwork(column_text_mapping, order_metas)
example_graph = E_est[0]
network.add_edges(example_graph, weight_thresh=0.1)
